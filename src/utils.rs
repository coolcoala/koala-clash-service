...
pub fn uninstall_old_service() -> Result<(), Error> {
    ...
    let target_binary_path = "/Library/PrivilegedHelperTools/io.github.koala-clash.helper";
    let plist_file = "/Library/LaunchDaemons/io.github.koala-clash.helper.plist";

    // Stop and unload service
    let _ = run_command("launchctl", &["stop", "io.github.koala-clash.helper"], false);
    let _ = run_command("launchctl", &["bootout", "system", plist_file], false);
    let _ = run_command(
        "launchctl",
        &["disable", "system/io.github.koala-clash.helper"],
        false,
    )?;
